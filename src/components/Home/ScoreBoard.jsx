            ...
            {scoreBoardData.map((item, index) => (
              <div key={item.id}>
                <motion.div
                  ...
                  viewport={{ once: true }} 
                  className={`w-56 h-72 rounded-lg shadow-lg relative card card${index + 1}`}
                >
                  ...
                  <div className={`absolute text-9xl font-bold top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-0 card-child-${index + 1}`}>
                    {index + 1}
                  </div>
                  
                  <div className="flex items-center justify-center w-full h-full z-10 text-center flex-col">
                    <h6 className={`text-3xl uppercase mt-20 font-bold card-team-${index + 1}`}>
                      {item.name === 'BSC' ? 'SCIENCE' : item.name}
                    </h6>
